...
# Exp 6:
# Aggregate embedder by avg, reduce graph to 2M + 1
# Fix z_M in server
...
class Server(BasicServer):
    ...
    def aggregate(self, models: list, modalities_list: list):
        ...
        modal_dict = dict()
        A = torch.zeros(size=(self.n_leads*2+1, n_models, n_models))   # attention matrix
        ...
        sum_z_M = sum([z.sum() for z in clients_z_M])
        new_model.z_M = torch.FloatTensor([sum(sub_list) / sum_z_M for sub_list in zip(*clients_z_M)])
        # p_k = 
