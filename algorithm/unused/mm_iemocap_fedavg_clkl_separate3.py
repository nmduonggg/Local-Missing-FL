...
class Server(BasicServer):
    def __init__(self, option, model, clients, test_data = None):
        ...
        self.combin_cnt = {
            "text+audio+vision": {
                "n_clients": sum([1 for client in self.clients if client.modal_combin == "text+audio+vision"]),
                "n_data": sum([client.datavol for client in self.clients if client.modal_combin == "text+audio+vision"])
            },
            "vision": {
    ...
    def aggregate(self, models: list, modal_combins: list):
        ...
        if self.combin_cnt["text+audio+vision"]["n_clients"] > 0:
            p = list()
            chosen_models = list()
            for cid, model, combin in zip(self.received_clients, models, modal_combins):
                if combin == "text+audio+vision":
...
class Client(BasicClient):
    ...
    def get_batch_data(self):
        ...
        try:
            ...
            self.data_loader = iter(self.calculator.get_data_loader(self.train_data, batch_size=self.batch_size, shuffle=True, num_workers=self.loader_num_workers))
            batch_data = next(self.data_loader)
