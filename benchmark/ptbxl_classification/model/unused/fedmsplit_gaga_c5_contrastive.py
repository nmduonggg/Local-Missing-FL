...
class Model(FModule):
    ...
    def forward(self, x, y, leads):
        ...
        features = torch.zeros(size=(batch_size, 128*2), dtype=torch.float32, device=y.device)
        extracted_features = torch.zeros(size=(batch_size, 128), dtype=torch.float32, device=y.device)
        relation_embeddings = torch.zeros(size=(batch_size, 128), dtype=torch.float32, device=y.device)
        ...
        for lead in total_lead_ind:    
            if lead in leads:
                ...
                relation_embedding = self.relation_embedders[lead](y.device).repeat(batch_size,1)        # 128, 256
                relation_embeddings += relation_embedding
        
        features = torch.cat((extracted_features,relation_embeddings),1)
