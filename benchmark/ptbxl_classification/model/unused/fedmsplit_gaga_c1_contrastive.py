...
class Model(FModule):
    ...
    def forward(self, x, y, leads):
        ...
        leads_features = []          # for contrastive
        for lead in total_lead_ind:    
            if lead in leads:
                lead_feat = self.feature_extractors[lead](x[:, lead, :].view(batch_size, 1, -1))
                features += lead_feat
                leads_features.append(lead_feat)
        ...
        feature_extractor_outputs = features
