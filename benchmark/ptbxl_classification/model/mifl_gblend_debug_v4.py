...
# Exp 4: 
# Exchange the position of embedder and extractor to make pipeline uniform in both missing and full cases
...
class RelationEmbedder(FModule):
    def __init__(self):
        ...
        self.relation_embedder = nn.Embedding(self.input_channels,250)
...
class Model(FModule):
    ...
    def forward(self, x, y, leads):
        ...
        for lead in total_lead_ind:    
            if lead in leads:
                relation_info = self.relation_embedders[lead](y.device, has_modal=True).repeat(batch_size, 1).unsqueeze(1)
                feature = self.feature_extractors[lead](x[:, lead, :].view(batch_size, 1, -1) + relation_info)
                ...
                relation_info = self.relation_embedders[lead](y.device, has_modal=False).repeat(batch_size,1).unsqueeze(1)        # 128, 256
                feature = self.feature_extractors[lead](relation_info)
                output = self.classifiers[lead](feature)
                loss_leads[lead] = self.criterion(output,y.type(torch.int64))
